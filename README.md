...
A wig in the closet is a saved set of codes; a fitting is that set proven on real hardware. The proving ground is the device, not a dialog: **adopt the wig** and use the thing. When it has earned your trust, press **Save to Closet** -- it asks what you mean first, with a one-line summary of how the device differs from its wig. **Save as New** files a fresh wig and leaves the original alone. **Update Closet Wig** brings your file up to match the device -- a simple edit when nothing changed, a replacement when it did, with the stakes named before you commit: which signals are added or removed, and whose fittings would retire. **Validate for Perfect Fit** is the fitting. A device that never came from a wig sees only Save and Perfect Fit; there is nothing to update.

The fitting is a checklist: every command a row, tick what you proved, with a **TEST** button on each that reports SENT, or SENT and HEARD when a receiver caught the transmission. A row your hardware refuses gets excluded honestly -- *could not make it work* -- recorded in standard form, so three people excluding the same row reads as a pattern rather than three sentences. If the device has grown or shrunk since the wig was saved, a **Changes with new fitting** section names exactly what this fitting adds and removes. Signing binds each row's transmit recipe by digest under a key generated on your install, so nobody can alter your verdicts or fit in your name -- identity is the key, not the typed name. Fix one bad code later and only that row's claims retire; everything else everyone proved still stands. Fit the same wig again and your new signing replaces your old one: one person, one current word.

Repairs never happen inside the fitting. A command that does not work gets its code replaced on the device, in the command editor -- paste a Pronto, or press **LISTEN** and capture it off the real remote -- and then saved: a changed device is a new description, so **Update Closet Wig** replaces your file with the corrected one, lineage carried. A matrix wig is the exception that repairs in place: its lattice changes propose upstream as an update, re-combed on the way out. Codes the comb flagged arrive on the device wearing the comb glyph, so you know which rows deserve suspicion before you vouch for them. One habit makes all of it go smoothly: be slow and deliberate -- give the device a beat between presses and a moment to visibly react before you judge.
