...
**Downloads carry the tier in the filename.** A downloaded file is named from the wig's own fields, `<brand>-<kind>-<model>`, each part slugified the same way filenames are (so `TH-05` becomes `th-05`) and any absent part skipped; a wig with no brand falls back to the slug of its name. The tier its check shows is then appended, hyphenated: nothing with no attestations, `-fitted` with signed but incomplete attestations, `-perfect-fit` when at least one person's claims cover every row -- for example `edifier-soundbar-r1280t-perfect-fit.wig.json`. The suffix is hyphenated rather than dotted so the name stays a single slug the shop's naming rule accepts. The name is presentation only -- the file contents are identical across tiers, an importing install never reads the name, and renaming the file changes nothing.
