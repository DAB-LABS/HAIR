...
- **HAIR Closet and Perfect Fit** - Portable code sets called wigs: one JSON file per remote, convertible on drop from SmartIR, Flipper Zero, LIRC, and Girr files. New in 0.8.0, wigs can be fitted: prove the codes on real hardware signal by signal, and the signed proof travels inside the file when you share it. Fitted wigs wear a green check in the closet.
- **Comb and Needs attention (new in 0.14.0 "Detangle")** - Every wig is combed on the way in: do its codes agree with each other? A device made from a flagged wig shows a Needs attention block above its commands, one row per code with a plain reason and a Fix button. Some fixes are ready to accept, some need a press from your remote, some ask you to pick between two buttons that share a name. Fixes are saved to your Closet as a repaired copy of the wig, and Perfect Fit waits until the block is empty.
- **Stateful air conditioners (new in 0.8.8 "Cold Cuts")** - SmartIR climate files convert into wigs carrying the full state matrix -- one complete code per mode / fan / swing / temperature combination -- and adopt as fully-controlled climate entities that transmit the exact state, the way AC remotes actually work. A STATE MATRIX card on the device page browses the lattice and saves any state as a command, and fitting a matrix takes 12 to 20 sends that stand in for the whole lattice. The honest edges: Xiaomi-controller Raw files are refused (a proprietary compressed format), a small fraction of corpus cells and modes with no Home Assistant equivalent are skipped with the reason noted on the wig, sparse matrices stay sparse, and files are treated as Celsius by convention unless they say otherwise.
