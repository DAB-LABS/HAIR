...
# Options-flow key: opt-in gate for user-authored, code-based AC protocols
# under <config>/hair_codes/ac/. See custom_ac_source.py -- executing
# arbitrary .py files from the config dir is a materially different risk
# than parsing YAML, so it defaults OFF and must be turned on explicitly
# in the integration's options.
OPT_ENABLE_CUSTOM_AC_PROTOCOLS = "enable_custom_ac_protocols"
