...
def test_device_type_to_platform_map():
    assert DEVICE_TYPE_TO_PLATFORM[DeviceType.TV] == "media_player"
    assert DEVICE_TYPE_TO_PLATFORM[DeviceType.SOUNDBAR] == "media_player"
    assert DEVICE_TYPE_TO_PLATFORM[DeviceType.PROJECTOR] == "media_player"
...
async def test_factory_dispatches_to_registered_platform(fake_hass):
    ...
    device = IRDevice(name="TV", device_type=DeviceType.TV)
