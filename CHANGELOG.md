...
- **The REPLACE column stopped staggering.** A row that had been tested grew a "sent" note inside itself, and because that note is longer or shorter depending on what happened, every tested row shoved its own buttons a different distance sideways. The note moved to its own line under the row, and the buttons now hold one straight column no matter what any row has been through.
