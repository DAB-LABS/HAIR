...
- The Plucker reads OpenIRBlaster code libraries. OpenIRBlaster keeps every code it learned in `.storage/openirblaster_<config entry id>`, and HAIR now imports them read-only, the same way it reads Broadlink and Tuya Local stores. From OpenIRBlaster 1.3.2 the file stays when the integration is removed, so it can still be plucked afterwards; on 1.3.1 and earlier, deleting the OpenIRBlaster entry deletes the file, so pluck first or update before removing it. That file is a flat list of codes rather than the subdevice map the other two use, so the whole library becomes one plucked remote. Each code keeps the carrier OpenIRBlaster measured when it was learned, which matters because a code re-learned through ESPHome comes back at 38 kHz; a code with no usable carrier falls back to 38 kHz and is recorded as assumed. An entry that cannot be read is counted in the landing summary and never costs the rest of the library. The store is named after the OpenIRBlaster config entry's title, found through the entry id in the filename (GH #169).
- Thinning: remove the states a unit does not have. A matrix wig imported from a file often carries combinations the hardware ignores or the handset never sends, and the only way to get rid of them was to repair cells one at a time or leave HAIR and edit the file. The STATE MATRIX card on a device now has an edit pencil. It turns the card into a list of lattices and modes where every value is a keep or remove toggle: narrow an axis, drop a mode, drop a whole preset lattice, or drop the separate On code. Off cannot be removed. Saving writes the device once and makes a new wig beside the original, the same way a repair does, so the original stays in the closet and re-adopting it is the undo. The save retires every fitting on the old lattice, including your own, and says so before you press it. A trimmed wig carries a record of what was removed, with no codes in it, and its card's summary line ends with what the trim cost ("trimmed from 143 to 77"). Comb portholes on removed cells go with them.
- The Save dialog's exclusion picker says where a state the unit does not have belongs: removed on the STATE MATRIX card, not excluded on the checklist.
...
- A state heard off the handset rings only in its own lattice on the remote's card. The card matched the heard state by coordinates against whichever lattice was on screen, so pressing Eco at cool / auto / 22 lit the main lattice's cool / auto / 22 tile, which is a different code.
