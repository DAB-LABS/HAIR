...
- **A wig now carries the whole recipe for sending its codes, not just the codes.** Some receivers refuse a lone frame: they want the pattern a real remote sends when you hold a button down, and a code rebuilt as one clean frame does nothing. That repeat count now travels in the wig, gets transmitted during a fitting, and is covered by the fitting's signature, so a code proven on your bench is proven the same way on someone else's. It is tunable from inside the fitting: nudge it, TEST it, and the thumb-up saves it into the wig and re-signs, carrying every other verdict forward. Nothing enters the file until something proves it, and closing without a verdict discards it.
- **Send counts stopped being part of a wig's identity, on purpose.** How many times to press is about your room, not about the device: distance, blaster power, the angle through a cabinet door. Five people proving the same codes at three, four and five sends are proving the same wig, so they now hash identically and their fittings pile up on one file instead of forking it five ways. The count still travels as the author's suggested floor, the fitting never sends fewer than the wig asks for, and the session control can only ever add on top. The SEND TIMES box gained an APPLY that writes your number onto every signal at once, which is how a device that wants three sends on everything gets set in one gesture instead of twenty.
- **Set a signal to send twice and you can now see that you did.** The send-count and NEC ditto markers that device commands have always shown now appear on Sniffer and Clipper signals too. Before this you could open a captured signal, tell it to fire three times, save, and find nothing on the row to say so; the setting was real but invisible everywhere except the device it eventually became. The markers follow the same rules in all four places they appear, including staying hidden on a code pinned to raw, where dittos never reach the air and showing a count for them would be a lie.
- **Combing runs itself the moment a device arrives.** Drop a file into the closet, or adopt one into a live device, and the code checker's report opens on its own. Those codes have never been checked against each other on your install, and finding out that 48 of them disagree is worth knowing before you start pressing buttons rather than after. It opens only when a single wig arrived, since one file in a foreign format can convert to five remotes at once.

### Changed

- **ADOPT DEVICE is now just ADOPT.** The noun was doing no work next to a device card, and the button was crowding the row it sits in. One label, so the Sniffer, Clipper, Plucker and Closet buttons all shortened together.
- **Wigs written by this release need this release.** The file's format stamp moved to `hair-wig/3` and older HAIRs refuse it with a message asking you to update. That refusal is the feature: the change to how a wig's identity is computed means an older install would otherwise compute the old answer, get a mismatch, and tell you a perfectly good file looked tampered with.
- **Fitting rows read as a checklist now.** SEND became TEST, and WORKED / DID NOT became a green thumb-up and a red thumb-down. The verdict rides the direction of the thumb rather than the colour, so it survives colourblindness, and the row no longer changes width depending on how long those two words are in your language.
- **The protocol chip moved off the command name and into a column.** In the device detail it sat beside the name, which made it the only list in HAIR that anchors the chip to text, so it slid left and right down the page as command names changed length. It now lines up with the edit glyph the way the Sniffer and Clipper chips already did.
- **Repeat frames are an NEC feature, and HAIR now says so.** The ditto count used to be offered on any signal HAIR could decode, on the assumption that repeat frames were a general idea. They are not. Only NEC appends the short repeat frame the setting describes; on Samsung and RC-5 the same number quietly duplicates the whole signal, which is what send times already do, and on Sharp and Sony it does nothing at all. The setting now appears only where it means something. Existing values are still shown wherever they were set, because a number you cannot see is a number you cannot correct.
- **The closet takes one file at a time.** It used to accept a whole handful, hang every one of them, and then tell you about the last. The other four arrived with no receipt and no mention. Dropping more than one now politely declines the lot, so nothing lands that you did not see land.
- **A fitting records how many times it really sent.** The attestation logged the send-times control's value, which was only half the story: a wig whose signals ask for three sends transmits three times even with the control at one. A candle proved at three sends was filed as proved at one. The number in the fitting is now the number that went out.

### Fixed

- **A fitting no longer proves a send it did not make.** Fitting sends were hardcoded to append no repeat frames, so a device that needs them would fail its own fitting while the wig's signature pinned a value nothing ever transmitted. The session now sends the row's stated recipe.
- **The send and ditto steppers close again.** Opening one left it open with no way back to the compact reading, so a fitting you had worked through ended up a row of half-finished controls. Click the number, click anywhere else, or press Escape. The tinted capsule around the open stepper is gone too; it made every row it touched look permanently mid-edit.
- **Buttons in a fitting row answer the mouse.** TEST, REPLACE and the thumbs had no hover and no pressed state, so nothing acknowledged a click. TEST has always been unavailable until you pick an emitter but looked exactly the same either way, and DISCARD gave no sign of whether there was anything to discard. All of them now show their state, and DISCARD wears red when it can actually act.
- **TEST tells you what it did.** Press it and the button itself turns green and reads SENT, or SENT and HEARD when one of your receivers caught the transmission, holds for five seconds so you can look at the device and back, then settles to TEST again. A grey dot in its corner counts how many times that row has been tried. This replaces a separate note beside the row, which was worse in two ways: its width changed with what happened, so every tested row shoved its own buttons a different distance sideways, and on air-conditioner wigs it was never drawn at all, which made TEST look like it was doing nothing on an entire class of device.
- **Air-conditioner fittings show their results.** Testing a cell on a state-matrix wig gave no feedback whatsoever, so the only way to know whether anything had been transmitted was to watch the appliance. Same button, same reporting as every other kind of remote now.
- **Every fitting row can be judged.** Codes the checker flags on an air conditioner sat in the list with no thumbs, because a flagged code is offered for repair rather than for judgement. On a wig where the checklist samples 31 of 288 states and 48 more are flagged, that meant most of what you were looking at could not be ticked off and there was no way to remember which ones you had already been through. They take a verdict now. What has not changed is the arithmetic: those rows still stay out of the tested-of-total count and out of the signed record, because running the code checker does not alter a single code, and letting it move the numbers would quietly demote finished fittings that other people signed.
- **Repairing an air-conditioner state no longer offers a choice it cannot keep.** The REPLACE panel let you pin a repaired cell to raw, but a state matrix has nowhere to record that, so the setting was accepted and discarded and the pill went on naming the protocol it had detected. The option is gone from matrix wigs, and the same request through the API is now refused rather than silently dropped.
- **Closet rows line up.** Library rows carry no DELETE and no comb glyph, and because a row's buttons are anchored to its right edge, every absence dragged the rest sideways: the download icons wandered by 64 pixels down the list and nothing shared a column with anything above it. The missing controls now keep their places, empty.
- **The protocol pill stopped offering a click it will not honour.** Its tooltip ended with an instruction to click no matter where it appeared, including inside a fitting, where changing a code would invalidate the attestation being written and is deliberately refused. It now says that only where it is true.
- **Code-checker findings sit next to their explanation.** The button name and the sentence diagnosing it were separated by a fixed gutter wide enough for the longest name any remote could produce, which on most reports meant reading across 90 pixels of empty space to connect the two. The column is now as wide as the names actually present.
- **A tuned signal no longer calls itself pasted.** The marker on a repaired row had two possible readings, captured or pasted, and tuning a repeat count wrote a third kind of mark that neither described. The chip picked the nearest match and claimed the code had been pasted in, about a code nobody had touched. Tuning now has its own mark, wearing the same glyph as the repeat setting it records.
- **Repairing and tuning the same signal keeps both facts.** Whichever you did second erased the first, so a code you captured off a real remote and later tuned came back with no record of where it came from. Both are kept now, and the row shows both.
- **The send-times APPLY button explains itself on air conditioners.** It has never been available there, on purpose: the checklist proves a sample of the states, so writing a send count from it would change states nothing in the session tested. The text under it went on describing what it would do anyway. It now says why it is off, and that the number above still applies to the session.
- **The fitting window is a little wider,** which is what stopped the signal names being squeezed once the row grew its send and ditto controls.
- **The code checker says what it skipped.** Combing has always left codes pinned to raw alone, correctly, and recorded that in the receipt. Nothing showed it, so a report listing two findings implied it had checked everything. Skipped rows now carry their own marker in the fitting list and a line in the report.
- **The Mirror now reports the repeats a send actually used.** A Mirror row is a log of what your house transmitted, but its repeat and ditto markers described whatever had been typed into that row rather than the command that produced it. A command set to send twice could sit under a Mirror row claiming three sends. Each send now records the counts it really used, so the audit and the transmission agree. Existing rows correct themselves the next time that signal is sent.
- **Rows stop shuffling sideways.** Several lists sized their columns to whatever text happened to be in them, so one signal heard "13h ago" instead of "4d ago", or one command with a longer mapped action, was enough to nudge the protocol chip and everything after it out of line down the whole list. The Sniffer's chip, hit count, time and frequency now sit in fixed columns, and the device detail's action badge reserves room for the widest label its device type can produce, so mapping an action no longer resizes the button under your cursor.
